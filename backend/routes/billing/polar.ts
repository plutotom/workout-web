...
function configuredBillingOrigins(): Set<string> {
  ...
  // Annotated so this still checks under the mobile tsconfig, which has no
  // @types/node and therefore sees `process.env` as untyped.
  const origins: string[] = configured?.split(",") ?? defaults;
