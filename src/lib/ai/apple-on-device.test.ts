...
const catalog = catalogExercisesForAi([
  ...
  { slug: "pullup", name: "Pull-Up", category: "back" },
  { slug: "chest-fly-db", name: "Chest Fly (Dumbbell)", category: "chest" },
...
describe("on-device prompts", () => {
  it("keeps create prompts well under the 4k window", () => {
    ...
    expect(built.requiredSlugs).toContain("bench");
  });

  it("pins an explicit user list in the prompt", () => {
    const built = buildOnDeviceTemplatePrompt({
      prompt: "squat, bench, pull up, and fly",
      mode: "create",
      catalog,
    });
    expect(built.requiredSlugs).toEqual([
      "squat",
      "bench",
      "pullup",
      "chest-fly-db",
    ...
    expect(built.prompt).toContain("chest-fly-db");
  });
...
describe("parseOnDevice drafts", () => {
  it("grounds template slugs and merges required lifts the model omitted", () => {
    const { draft, droppedSlugs } = parseOnDeviceTemplateDraft(
      ...
      new Set(["bench", "squat", "pullup", "chest-fly-db"]),
      {
        prompt: "squat, bench, pull up, and fly",
        requiredSlugs: ["squat", "bench", "pullup", "chest-fly-db"],
      },
    );
  ...
  it("grounds session removals to the current session", () => {
    ...
    expect(draft.add[0]?.sets).toHaveLength(3);
  });
});
