...
**Set `APPLE_TEAM_ID`** (10-character Apple Developer Team ID — not a secret) in
the Vercel environment. Without it the route returns 404 and universal links
silently fall back to the browser; the in-app `workout://share/<token>` scheme
and the file/code transports still work. Override the bundle id with
`APPLE_BUNDLE_ID` if it changes.
