...
struct WorkoutTemplateDraft {
  ...
  var name: String
  @Guide(
    description: "Ordered exercises. Include every lift the user named (often 5–7). Never stop at 3 if they named more.",
    .count(1...12)
  )
...
struct WorkoutExerciseDraft {
  ...
  var slug: String
  @Guide(
    description: "Working set rows. Default to 3 identical sets (weight 0, reps 0). Use 1 row only if the user asked for a single set. Expand 4x8 into 4 rows.",
    .count(1...8)
  )
...
struct WorkoutSessionDraft {
  @Guide(
    description: "Existing session exercise slugs to remove. Empty if the user only wants additions.",
    .maximumCount(12)
  )
  var removeSlugs: [String]
  @Guide(
    description: "New exercises to add. Include every lift the user named. Empty if the user only wants removals.",
    .maximumCount(12)
  )
